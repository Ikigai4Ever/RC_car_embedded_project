...
#define KP 6.5f     // 0.6 * 10 (your Ku)
#define KI 3.0f     // Start here, increase if steady-state error persists
#define KD 0.04f     // Start with 0, add only if needed for overshoot
...
typedef struct
{
    ...
    volatile int32_t encoder_speed_raw;      // Unfiltered speed
    volatile float encoder_speed_filtered;   // Filtered speed for PID
    volatile int32_t last_encoder_count;

    // PID variables
    volatile float target_speed;        // Desired speed in counts per 0.01s (can be negative)
    volatile float desired_speed;       // User's pot input (ramps to this)
    volatile float pid_error;
    volatile float pid_integral;
    volatile float pid_last_error;
    volatile float pid_output;          // PID output in PWM units (0 to TIM_ARR)

    // Diagnostic variables - WATCH THESE IN LIVE EXPRESSIONS
    volatile float p_term;  // KP * error
    volatile float i_term;  // KI * integral
    volatile float d_term;  // KD * derivative
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
// Ramp the target speed smoothly to avoid jerky transitions
// Ramp the target speed smoothly to avoid jerky transitions
...
void update_encoder_speed(Motor *motor, int32_t current_encoder_count)
{
    ...
    if (count_diff > 32768)
    {
        count_diff -= 65536;  // Wrapped from high to low
    }
    else if (count_diff < -32768)
    {
        count_diff += 65536;  // Wrapped from low to high
    }

    // FIX: Negate the encoder speed because your encoders are wired backwards
    ...
    // Use filtered value as the final encoder speed
...
{
    // If target speed is zero, reset PID and set output to zero
    ...
    // Calculate error directly in encoder speed units (counts per 0.01s)
    float target_encoder_speed = fabs(motor->target_speed);  // 0 to 100
    float actual_encoder_speed = fabs((float)motor->encoder_speed);  // 0 to 100

    // Error in encoder speed units
    motor->pid_error = target_encoder_speed - actual_encoder_speed;

    // Update integral (with anti-windup)
    motor->pid_integral += motor->pid_error * 0.01f; // dt = 0.01s (100Hz)
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
    ...
    // Calculate base speed from pot (this becomes our DESIRED encoder speed)
    speed_base = (speed * TIM_ARR) / ADC_MAX;

    // Convert speed_base to desired encoder speed
    // speed_base is 0 to TIM_ARR, we need to map to encoder speed range
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == TIM6) // 100Hz update
    {
        ...
        // CALIBRATION MODE: Run at full PWM to find ENCODER_MAX_SPEED
        if (calibration_mode == 1)
        {
            motor1.ch1_pwm = TIM_ARR;  // Full speed forward
            motor1.ch2_pwm = 0;
            motor2.ch1_pwm = TIM_ARR;
            motor2.ch2_pwm = 0;
            ...
            // Watch motor1.encoder_speed and motor2.encoder_speed in live expressions!
            // The peak value is your ENCODER_MAX_SPEED
            return;  // Skip PID in calibration mode
        }
        ...
        // SAFETY: Block opposite direction until current direction finishes ramping down
        // Motor 1 safety
        if (motor1.ch1_pwm > 10 && motor1.target_ch2 > 0)
        {
            // Still going forward, don't allow reverse yet
            motor1.target_ch2 = 0;
        }
        if (motor1.ch2_pwm > 10 && motor1.target_ch1 > 0)
        {
            // Still going reverse, don't allow forward yet
            motor1.target_ch1 = 0;
        }
        ...
        // PID directly controls PWM (no ramp needed, PID handles smoothing)
